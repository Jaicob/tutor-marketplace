var SlotSelector = React.createClass({
    ...
    slots = slots.map(function (slot) {
      ...
      rangeTimeOfSlot.by('minutes', function (moment) {
        if (moment.minutes() % 30 == 0) {
          // checks if starting at either a half hour or a full hour
  ...
  getDisabledSlots: function (selectedSlots) {
    selectedTimeRanges = selectedSlots.map(function (slot) {
      futureNearbySlots = moment.range(moment(slot.start_time).add({ minutes: 1 }), moment(slot.start_time).add({ minutes: 59 }));
      pastNearbySlots = moment.range(moment(slot.start_time).subtract({ minutes: 59 }), moment(slot.start_time).subtract({ minutes: 1 }));
      return [futureNearbySlots, pastNearbySlots];
    });

    if (selectedTimeRanges.length > 0) {
      selectedTimeRanges = selectedTimeRanges.reduce(function (finalRange, partialRange) {
        return finalRange.concat(partialRange);
      });
    };
    ...
    disabledSlots = disabledSlots.filter(function (item, pos) {
      return disabledSlots.indexOf(item) == pos;
    });
  ...
  handleSlotClick: function (slot, active) {
    ...
    if (active) {
      ...
      newSelectedSlots = newSelectedSlots.filter(function (targetSlot) {
        return slot.start_time != targetSlot.start_time;
      });
  ...
  renderSlotDays: function () {
    ...
    moment.range([this.state.startRange, this.state.endRange]).by("day", (function (day) {
      ...
      result.push(React.createElement(Day, { day: day, slots: slots, selectedSlots: this.props.selectedSlots, disabledSlots: this.props.disabledSlots, handleSlotClick: this.handleSlotClick }));
    }).bind(this));
    return result;
  },
  render: function () {
    return React.createElement(
      'div',
      null,
      React.createElement(SlotRangeControls, { handleNextRange: this.handleNextRange,
        handlePreviousRange: this.handlePreviousRange
      }),
      React.createElement(
        'ul',
        { className: "schedule weekly-schedule" },
        this.renderSlotDays()
      )
    );
  }
