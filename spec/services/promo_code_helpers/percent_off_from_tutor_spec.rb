...
RSpec.describe 'PromoCodeHelpers::PercentOffFromTutor' do
  ...
  describe 'Methods in PromoCodeServices::PercentOffFromTutor' do
    ...
    it 'increments the redemption_count for a promotion by 1 when succesfully applied' do
      @promotion = create(:promotion, category: :percent_off_from_tutor, amount: 20)
      params = {
        tutor: tutor,
        appointments: [appointment],
        customer_id: 1,
    ...
    it 'does not give discount for expired promo_code' do
      @promotion = create(:promotion, category: :percent_off_from_tutor, amount: 20, valid_from: Date.today - 300, valid_until: Date.today - 299) 
      params = {
        tutor: tutor,
        appointments: [appointment],
        customer_id: 1,
    ...
    it 'does not give discount for promo_code if code is past redemption_limit' do 
      @promotion = create(:promotion, category: :percent_off_from_tutor, amount: 20, redemption_limit: 100, redemption_count: 100)
      params = {
        tutor: tutor,
        appointments: [appointment],
        customer_id: 1,
