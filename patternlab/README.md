## About *this* Pattern Lab
- This is the default pattern library being used on Proxyserver.com.
- I've removed most of the bitmap images to keep the repo size down.
- To start this PatternLab you need to have the ability to run PHP, NPM (requires Node.js) and Grunt (npm install grunt -g).
- Use the following steps to initialize this Pattern Lab build:
  * From the Pattern Lab root ("patternlab/")" directory run "php core/builder -gp"  to initialize the build and create the "public/" folder.
  * Run "npm install --save-dev" (might require sudo).  This will install the necessary the necessary node modules for the build.
  * Run "grunt" to start the watcher. 
    * Start a local server at http://localhost:8081
    * Watch JSON, Mustache, JS, SASS files for changes
    * SASS compile/concatenate
    * JS concatenate/uglify/mangle
    * rsync CSS/SASS/JS files to the "template/" directory
    * run the Pattern Lab generator to update the libaries

*thx! - [@torchcodelab](https://twitter.com/torchcodelab)*
